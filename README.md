...
##### Network 
###### Network Card
A network card is a device used to connect a computer to a netowrk. The card operates by sending data via a physical connection, the most common of which being an ethernet connection. Computers can also be connected to eachother wirelessly, this is usually achieved by using Wi-Fi. Wi-Fi is the most common way to achieve wireless network access. It used radio waves send at a certain frequency in order to transmit data. The frequencies that Wi-Fi can operate on are 2.4Ghz and 5Ghz. 2.4Ghz usually delivers faster speeds however physical objects such as walls can get in the way of the signal and weaken it. On the other hand 5Ghz can be a little slower but the signal can penetrate barriers without losing much if any signal strength, depending on what is in the way of the signal. A network card is the only example of where a physical medium is needed in order to connect 2 or more computers.
